...
const doUsuario = (db, req) => db.listas.filter(l => l.usuarioId === req.userId);

exports.listar = (req, res) => res.json(doUsuario(store.read(), req));

exports.criar = (req, res) => {
  const { titulo, cor_hex } = req.body;
  if (!titulo || !titulo.trim()) return res.status(400).json({ erro: 'O título da lista é obrigatório.' });
  const db = store.read();
  const lista = { id: store.novoId('l'), usuarioId: req.userId, titulo: titulo.trim(), cor_hex: cor_hex || '#4F86C6', criadaEm: new Date().toISOString(), tarefas: [] };
  db.listas.push(lista); store.write(db); res.status(201).json(lista);
};

exports.atualizar = (req, res) => {
  const db = store.read();
  const lista = db.listas.find(l => l.id === req.params.id && l.usuarioId === req.userId);
...
exports.reordenar = (req, res) => {
  ...
  store.write(db); res.json(doUsuario(db, req));
};
